        ...
        for interaction in interactions
            for pin in interaction.get_in_particles(), pout in interaction.get_out_particles()
                ...
                for type in interaction.get_types(pin,pout)
                    ...
                    @time Σsli, Σti, Σai, Σei, Σci, Sbi, Si, Ti, Tbi = multigroup(Z[n],ωz[n],ρ[n],state_of_matter[n],Eᵇ[i],Eᵇ[j],L,interaction,type,particles[i],particles[j],particles,interactions,I_eff[n],A[n],atpercentA[n])
